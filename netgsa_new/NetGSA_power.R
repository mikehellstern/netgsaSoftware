library(netgsa, lib.loc = "/home/students/mikeh1/rlib/R_4_0_1")
setwd("")
...
calcPower <- function(muval, iter, dysreg, dataset, dataset_name, penalize_diag){
  ...
  ## Estimating network
  ##*******************************
  cat(paste0("..... ", "Iteration: ", iter, " Estimating Adj with Cluster = TRUE ....\n"))
  start_adj_cluster_T <- Sys.time()
      adj_info_cluster_T <- prepareAdjMat(dysreg_dat_full, group = dataset$gene_groups, databases = dataset$database_edges, cluster = TRUE, lambda_c = dataset$lambda_cluster, penalize_diag = penalize_diag)
  stop_adj_cluster_T  <- Sys.time()
  time_adj_cluster_T <- stop_adj_cluster_T - start_adj_cluster_T
  ...
  cat(paste0("..... ", "Iteration: ", iter, " Estimating Network with Cluster = TRUE ....\n"))
  start_net_cluster_T <- Sys.time()
      out_cluster_T  <- NetGSA(A = adj_info_cluster_T[["Adj"]],  x = dysreg_dat_full,  group=dataset$gene_groups, pathways = dataset$pathway_matrix, lklMethod = "REHE", sampling = TRUE, sample_n = 0.1, sample_p = 0.1)
  stop_net_cluster_T <- Sys.time()
  time_net_cluster_T <- stop_net_cluster_T - start_net_cluster_T
  
  cat(paste0("..... ", "Iteration: ", iter, " Estimating Network with Cluster = FALSE ....\n"))
  start_net_cluster_F <- Sys.time()
      out_cluster_F  <- NetGSA(A = adj_info_cluster_F[["Adj"]],  x = dysreg_dat_full,  group=dataset$gene_groups, pathways = dataset$pathway_matrix, lklMethod = "REHE", sampling = TRUE, sample_n = 0.1, sample_p = 0.1)
  stop_net_cluster_F <- Sys.time()
  time_net_cluster_F <- stop_net_cluster_F - start_net_cluster_F
  
  timing_df <- data.frame(seed = iter, dysregulation_framework = dysreg, mu = muval, dataset_name = dataset_name, penalize_diag = penalize_diag,
                          time_adj_cluster_T = time_adj_cluster_T, time_adj_cluster_F = time_adj_cluster_F, time_net_cluster_T= time_net_cluster_T, time_net_cluster_F = time_net_cluster_F)
  ...
  return(list(results_cluster    = out_cluster_T[c("results", "beta", "s2.epsilon", "s2.gamma")], 
              results_noncluster = out_cluster_F[c("results", "beta", "s2.epsilon", "s2.gamma")], 
              timing = timing_df,
              seed = iter,
...
curr_data <- (if (dataset_name == "BreastCancer"){readRDS("./Inputs/breastcancer2012.rds")}
              ...
              else if(dataset_name == "Metabolites") {readRDS("./Inputs/metabolites.rds")})
...
save_path <- "./Power results/"
saveRDS(power_one_iteration, paste0(save_path, file_save_name, ".rds"))
